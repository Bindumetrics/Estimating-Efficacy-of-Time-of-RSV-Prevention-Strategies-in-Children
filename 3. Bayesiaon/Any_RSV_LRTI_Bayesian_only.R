# ============================================================
# Bayesian-Only Waning Efficacy Analysis - Any RSV LRTI, Extended to 360 Days
# Fixed version for a SINGLE RDS file: Any_RSV_LRTI.rds
...
#   2. Does NOT assume data_list$placebo or data_list$Maternal_Vaccine exist.
...
# Your single R data file
DATA_FILE <- file.path(DATA_DIR, "Any_RSV_LRTI.rds")

# Output folder for graphs and saved results
OUT_DIR <- file.path(DATA_DIR, "analysis_outputs_Any_RSV_LRTI_360_DAYS_BAYESIAN_ONLY")
...
# Maximum follow-up horizon for the extended dataset, in days.
ANALYSIS_T_MAX <- 360
...
standardize_arm <- function(x) {
  ...
  out <- ifelse(grepl("plac|control|standard", z), "placebo",
                ifelse(grepl("nir|nirse|treat|active|mab", z), "Maternal_Vaccine", z))
  return(out)
}
...
make_survival_from_ipd <- function(ipd) {
  ...
  if (!all(c("placebo", "Maternal_Vaccine") %in% unique(tmp$arm))) {
...
# Keep only placebo and Maternal_Vaccine arms.
km_all <- km_all[km_all$arm %in% c("placebo", "Maternal_Vaccine"), ]

if (!all(c("placebo", "Maternal_Vaccine") %in% unique(km_all$arm))) {
  stop("After extracting KM data, both placebo and Maternal_Vaccine arms were not found. Arms found: ",
       paste(unique(km_all$arm), collapse = ", "))
}

km_pla <- add_zero_anchor(km_all[km_all$arm == "placebo", ])
km_nir <- add_zero_anchor(km_all[km_all$arm == "Maternal_Vaccine", ])
...
# Follow-up horizon.
# The dataset is extended to 360 days; all plots, model bounds, and summaries use 0-360 days.
T_max_data <- max(c(km_pla$t, km_nir$t), na.rm = TRUE)

if (!is.null(data_list$t_max)) {
  T_from_object <- clean_days(data_list$t_max)[1]
  if (is.finite(T_from_object) && T_from_object > 0) {
    T_max_data <- max(T_max_data, T_from_object, na.rm = TRUE)
  }
}

T_max <- ANALYSIS_T_MAX

if (!is.finite(T_max) || T_max <= 0) {
  stop("T_max is not positive. Check time variables in km_clean/ipd.")
}

if (T_max_data < ANALYSIS_T_MAX) {
  warning("The observed data maximum is only ", T_max_data,
          " days, but ANALYSIS_T_MAX is set to ", ANALYSIS_T_MAX,
          " days.")
}

cat("\nFollow-up horizon T_max =", T_max, "days\n")
cat("Placebo KM rows:", nrow(km_pla), "\n")
cat("Maternal_Vaccine KM rows:", nrow(km_nir), "\n")

n_by_arm <- get_n_by_arm(data_list, km_all)
N_pla <- as.numeric(n_by_arm["placebo"])
N_nir <- as.numeric(n_by_arm["Maternal_Vaccine"])

cat("\nSample sizes:\n")
cat("  Placebo    =", N_pla, "\n")
cat("  Maternal_Vaccine =", N_nir, "\n")
...
p_km <- ggplot(km_plot_data,
  ...
  labs(title = "Kaplan-Meier Curves - Any RSV LRTI",
       x = "Days since dosing",
       y = "Proportion free of Any RSV LRTI",
       color = "Arm") +
  theme_bw() +
...
cat("\nMaternal_Vaccine events head:\n")
print(head(events_nir, 10))
cat("Total Maternal_Vaccine infections:", sum(events_nir$n[events_nir$Type == "I"]), "\n")
...
recon_plot_data <- rbind(
  ...
  data.frame(km_nir[, c("t", "Proportion_Free")], series = "Maternal_Vaccine original"),
  data.frame(data_pla_clean$KM_estimator, series = "Placebo reconstructed"),
  data.frame(data_nir_clean$KM_estimator, series = "Maternal_Vaccine reconstructed")
)
...
print(head(agg_pla))
cat("\nAggregated Maternal_Vaccine data head:\n")
print(head(agg_nir))

agg_plot <- rbind(
  data.frame(agg_pla, arm = "Placebo"),
  data.frame(agg_nir, arm = "Maternal_Vaccine")
)
...
# ============================================================
# SECTION 9: BAYESIAN MODEL DEFINITION ONLY
# ============================================================
...
write.csv(all_param_summaries, file.path(OUT_DIR, "Bayesian_parameter_summaries.csv"), row.names = FALSE)

# ============================================================
# SECTION 12: ATTACK RATE PLOTS
# ============================================================

ts_plot <- seq(0, T_max, by = DT)
...
for (k in models_to_run) {
  ...
  p_ar_pla <- ggplot() +
    ...
    coord_cartesian(xlim = c(0, T_max), ylim = c(0, NA)) +
    labs(x = "Days after birth", y = "Attack rate per person-day",
  ...
  if (!is.null(model$waning)) {
    ...
    p_ar_nir <- ggplot() +
      ...
      coord_cartesian(xlim = c(0, T_max), ylim = c(0, NA)) +
      labs(x = "Days after birth", y = "Attack rate per person-day",
           title = paste("Maternal_Vaccine attack rate -", model$name)) +
      theme_bw()

    save_plot(p_ar_nir, paste0("06_Maternal_Vaccine_attack_rate_", model$name, ".png"))
  }
}
...
# ============================================================
# SECTION 14: BAYESIAN EFFICACY OUTPUT
# ============================================================

# Only the Bayesian model is retained, so no multi-model comparison plot is produced.
# The Bayesian efficacy curve is already saved in Section 13.
...
summary_table <- data.frame(
  ...
  Mean_Eff_D0 = sapply(treatment_models, function(k)
    get_eff_at_day(models[[k]]$efficacy_results, 0)),
  Mean_Eff_D30 = sapply(treatment_models, function(k)
    get_eff_at_day(models[[k]]$efficacy_results, 30)),
  Mean_Eff_D90 = sapply(treatment_models, function(k)
    get_eff_at_day(models[[k]]$efficacy_results, 90)),
  Mean_Eff_D180 = sapply(treatment_models, function(k)
    get_eff_at_day(models[[k]]$efficacy_results, 180)),
  Mean_Eff_D270 = sapply(treatment_models, function(k)
    get_eff_at_day(models[[k]]$efficacy_results, 270)),
  Mean_Eff_D360 = sapply(treatment_models, function(k)
    get_eff_at_day(models[[k]]$efficacy_results, 360)),
  Mean_Eff_End = sapply(treatment_models, function(k)
    round(tail(models[[k]]$efficacy_results$mean, 1), 3))
)

cat("\n===== EFFICACY SUMMARY TABLE =====\n")
print(summary_table)
write.csv(summary_table, file.path(OUT_DIR, "Bayesian_efficacy_summary_table.csv"), row.names = FALSE)
...
save(models, agg_pla, agg_nir, data_pla_clean, data_nir_clean,
     ...
     file = file.path(OUT_DIR, "RSV_LRTI_Bayesian_only_results.RData"))
